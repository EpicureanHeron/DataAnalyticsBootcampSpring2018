...
FROM
    country
WHERE
    country.country IN ('Afghanistan' , 'Bangladesh', 'China');
...
SELECT 
    title, COUNT(rental.inventory_id) AS Times_Rented
FROM
    film
        INNER JOIN
...
SELECT 
    store.store_id, SUM(payment.amount) AS Revenue
FROM
    store
        INNER JOIN
